...
altair_form_adv = {
            ...
            (function ($) {
                ...
                $.fn.extend({
                    counter: function (options) {
                        var defaults = {
                            type: "char", count: "down", goal: 140, text: true,
                            target: false, append: true, translation: "", msg: "",
                            container_class: ""
                        };
    ...
    date_range: function () {
        ...
        ///
        var $dp_start2 = $('#FromDate'),
            $dp_end2 = $('#ToDate');

        var start_date2 = UIkit.datepicker($dp_start2, {
            format: 'DD-MM-YYYY'
        });

        var end_date2 = UIkit.datepicker($dp_end2, {
            format: 'DD-MM-YYYY'
        });

        $dp_start2.on('change', function () {
            end_date2.options.minDate = $dp_start2.val();
            setTimeout(function () {
                $dp_end2.focus();
            }, 300);
        });

        $dp_end2.on('change', function () {
            start_date2.options.maxDate = $dp_end2.val();
        });
        /////////////////////////

        var $dp_start3 = $('#FromDateChange'),
            $dp_end3 = $('#ToDateChange');

        var start_date3 = UIkit.datepicker($dp_start3, {
            format: 'DD-MM-YYYY'
        });

        var end_date3 = UIkit.datepicker($dp_end3, {
            format: 'DD-MM-YYYY'
        });

        $dp_start3.on('change', function () {
            end_date3.options.minDate = $dp_start3.val();
            setTimeout(function () {
                $dp_end3.focus();
            }, 300);
        });

        $dp_end3.on('change', function () {
            start_date3.options.maxDate = $dp_end3.val();
        });
        /////////////////////////

        var $dp_start4 = $('#FromDateChange2'),
            $dp_end4 = $('#ToDateChange2');

        var start_date4 = UIkit.datepicker($dp_start4, {
            format: 'DD-MM-YYYY'
        });

        var end_date4 = UIkit.datepicker($dp_end4, {
            format: 'DD-MM-YYYY'
        });

        $dp_start4.on('change', function () {
            end_date4.options.minDate = $dp_start4.val();
            setTimeout(function () {
                $dp_end4.focus();
            }, 300);
        });

        $dp_end4.on('change', function () {
            start_date4.options.maxDate = $dp_end4.val();
        });
        ////////////////////////

        var $dp_start5 = $('.FromRangeDate'),
            $dp_end5 = $('.ToRangeDate');

        var start_date5 = UIkit.datepicker($dp_start5, {
            format: 'DD-MM-YYYY'
        });

        var end_date5 = UIkit.datepicker($dp_end5, {
            format: 'DD-MM-YYYY'
        });

        $dp_start5.on('change', function () {
            end_date5.options.minDate = $dp_start5.val();
            setTimeout(function () {
                $dp_end5.focus();
            }, 300);
        });

        $dp_end5.on('change', function () {
            start_date5.options.maxDate = $dp_end5.val();
        });
        ////////////////////////

        var $dp_start6 = $('.FromRangeDate2'),
            $dp_end6 = $('.ToRangeDate2');

        var start_date6 = UIkit.datepicker($dp_start6, {
            format: 'DD-MM-YYYY'
        });

        var end_date6 = UIkit.datepicker($dp_end6, {
            format: 'DD-MM-YYYY'
        });

        $dp_start6.on('change', function () {
            end_date6.options.minDate = $dp_start6.val();
            setTimeout(function () {
                $dp_end6.focus();
            }, 300);
        });

        $dp_end6.on('change', function () {
            start_date6.options.maxDate = $dp_end6.val();
        });
    }
};